...
jojo.createServer = function () {
  ...
  var express = require('express'),
      server = express(arguments);
  jojo.express = express;
...
function sendArticle(req, res, next) {
  ...
  yoyo.readArticles(null, function (err, articles) {
    ...
    var articleUrl = req.params.article,
        i = 0,
        len = articles.length,
        article,
        articleFound = false;
...
function sendRss(req, res, next) {
  ...
  yoyo.readArticles(null, function (err, articles) {
    ...
    var feed = generateRss(articles, req);
    res.type('rss');
    res.send(feed.xml());
    // Otherwise, render
...
function generateRss(articles, req) {
    console.dir(articles[0]);
    var feed = new rss({
        title: 'Jazz',
        description: 'get some cookies',
    ...
    articles.forEach(function( post ) {
        feed.item({
            ...
            url: 'http://' + req.headers.host + '/' + post.path,
            author: post.author,
            date: post.date
        });
    });
...
Yoyo.prototype = {
  'ignoreTmpl': function (articles, tmpl) {
    var ax = articles.indexOf(tmpl);
    if (ax !== -1) {
      articles.splice(articles.indexOf(tmpl), 1);
    }
  },
...
//      that.ignoreTmpl(articles, 'tmpl.md');
